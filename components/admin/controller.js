...
adminCtrl.registAdmin = async (name, email, password)=>{
	try{
		//hastear contraseña
...
adminCtrl.loginAdmin = async (req)=>{
	try{
		//verificar email
...
adminCtrl.checkEmailAvailable = async (email)=>{
	try{
		const user = await modelo.findOne({email:email})
		if (user) {
...
adminCtrl.updateAdmin = async (_id, name, email, rol,res)=>{
	try{
...
adminCtrl.deleteAdmin = async (_id,res)=>{
	try{
